<template>
  <div id="editorCanvas">
    ...
    <div class="button-menu" :style="showMenu ? 'display: block;' : 'display: none;'">
      ...
      <button @click="flushEditorData">Flush</button>
    </div>
  </div>
</template>
  ...
  created() {
    ...
    this.args.base_blocks.forEach((el) => {
      ...
      if (!this.args.load_editor_schema) {
        ...
        block.name = block.name + "-" + this.BlockNameID[el.name]++;
        const len_input = Object.keys(block.inputInterfaces).length;
        const len_output = Object.keys(block.outputInterfaces).length;
        this.editor.addNode(block);
        // Set the position of the block

        if (len_input > 0) {
          if (len_output > 0) {
            // 有输入和输出
            block.position.x = 270;
            block.position.y = 150 * mi;
            mi += 1;
          } else {
            // 只有输入
            block.position.x = 520;
            block.position.y = 150 * ri;
            ri += 1;
          }
        } else {
          // 只有输出
          block.position.x = 20;
          block.position.y = 150 * li;
          li += 1;
        }
      }
    });
